...
export default function EditPostPage() {
  ...
  // 👉 2. TẢI DỮ LIỆU BÀI VIẾT CŨ THEO SLUG
  useEffect(() => {
    const fetchPostData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setForm({
            title: p.title || "",
            category: p.category || "", // Lấy category cũ
            summary: p.summary || "",
            content: p.content || "",
          });
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // NHỚ THAY THÔNG TIN CLOUDINARY VÀO ĐÂY NHÉ
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.category) return alert("Vui lòng chọn Chuyên mục cho bài viết!"); // Cảnh báo nếu chưa chọn
    if (!form.content || form.content === '<p><br></p>') return alert("Vui lòng nhập nội dung bài viết!");
    
    setIsSubmitting(true);
    try {
      const dataToSend = { ...form, imageUrl: preview }; 
  ...
  if (isLoadingData) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col justify-center items-center">
        ...
        <p className="text-amber-500 font-bold uppercase tracking-widest text-xs animate-pulse">Đang nạp dữ liệu bản thảo...</p>
      </div>
    );
  }
    ...
    <main className="min-h-screen bg-[#F8FAFC] text-[#0F172A] pb-24 relative selection:bg-amber-100">
        ...
        {showSuccess && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center px-4 bg-slate-900/80 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white rounded-[3rem] p-12 max-w-md w-full text-center shadow-2xl border border-white">
              ...
              <h3 className="text-3xl font-black mb-4 tracking-tighter uppercase">Cập Nhật Xong!</h3>
              <p className="text-slate-500 mb-10 font-medium text-lg italic">Bản thảo <span className="text-amber-600 font-bold">"{form.title}"</span> đã được lưu.</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-5xl mx-auto px-6 pt-12">
        <div className="mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-tight">Chỉnh Sửa Bài Viết</h1>
          <p className="text-slate-500 mt-3 text-lg">Đang chỉnh sửa bản thảo: {postSlug}</p>
        </div>
              ...
              {!preview ? (
                <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full aspect-[16/9] rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center hover:bg-amber-50 hover:border-amber-400 hover:text-amber-600 transition-all duration-300 group">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-slate-500 group-hover:text-amber-600">
                    {isSubmitting ? "Đang tải lên..." : "Tải ảnh mới"}
                  </span>
          ...
          <section>
            ...
            <div className="flex flex-col gap-8">
              <div className="flex flex-col gap-2">
                ...
                <input type="text" className="w-full px-5 py-4 rounded-xl border-2 border-slate-200 outline-none transition-all focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 text-slate-900 text-lg font-bold" value={form.title} onChange={(e) => setForm({...form, title: e.target.value})} required />
              </div>
              
              {/* 👉 3. CHỌN DANH MỤC TỪ DATABASE */}
              <div className="flex flex-col gap-2 md:w-1/3">
                ...
                >
                  ...
                  {/* Lọc và map các danh mục có type là 'post' từ Database */}
              ...
              <div className="flex flex-col gap-2">
                ...
                <textarea rows={3} className="w-full bg-slate-50 px-5 py-4 rounded-xl border-2 border-slate-200 outline-none text-slate-700 focus:bg-white focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all resize-none" required value={form.summary} onChange={(e) => setForm({...form, summary: e.target.value})} />
              </div>
            </div>
          </section>
